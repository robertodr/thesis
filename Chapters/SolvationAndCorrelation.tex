...
\begin{equation}\label{eq:third-order-G}
  \begin{aligned}
    G^{[3]} &=
    \sum_j\braket{\tbar{j}^{[1]} | \commutator{\Phi}{T^{[1]}} | \mathrm{HF}}
...
\todo[inline]{Finish up on CC2 and CC3.}
...
Starting from the \acrshort{CC2} and \acrshort{CC3} Lagrangians, Eqs.
\eqref{eq:cc2-lagrangian} and \eqref{eq:cc3-lagrangian} respectively,
we can write down the corresponding effective Lagrangians. The correct
source term $\s_\mathrm{N}(\tamp{},\tbar{})$ must however be identified.
For the \acrshort*{CC2} model, we start from its \acrshort*{CCSD}
expression, Eq. \eqref{eq:source-ccsd}, and retain only terms up to first order:
\begin{equation}
  \begin{aligned}
  \s_{\mathrm{N}}(\tamp{}, \tbar{})_\mathrm{CCSD} &=
  \s_{\mathrm{N}}(\tamp{}, \tbar{})_\mathrm{CCS}
  ...
  \end{aligned}
\end{equation}
where the $T_1$-transformed operator was used.
...
\begin{subequations}\label{eq:g-parentheses-t}
 \begin{align}
  G_{\Lambda(\text{T})}
  &=
    \sum_{j}\braket{\tbar{j}^{*}| \commutator{\Phi}{T_3^{*}} | \mathrm{HF}}
    + {}^t\p^{*}\braket{\tbar{2}^{*} | \commutator{\s}{T_3^{*}} | \mathrm{HF}} \\
  G_{(\text{T})}
 &=
    \sum_{j}\braket{\tamp{j}^{*}|\commutator{\Phi}{T_3^{*}} | \mathrm{HF}}
