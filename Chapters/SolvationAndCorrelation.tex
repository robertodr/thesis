...
\epigraph{Potius sero quam numquam}{
--- \textsc{Titus Livius}, \textit{Ab Urbe Condita}}
...
The first approximate scheme\footnote{Thanks to prof. Marco Caricato
(University of Kansas) for clarifying some details of his original
derivation of the \gls{PTES} scheme.} truncates the source term to its
...
\begin{subequations}\label{eq:ptes-amp}
  \begin{align}
   ...
   \denom{\mu_q}\tamp{\mu_q} + \braket{\mu_q | \overline{\Phi} |
   \mathrm{HF}} \nonumber \\
   &+ {}^t\p\braket{\mu_q | \check{\s} | \mathrm{HF}}\delta_{\mu_q\mu_1}
...
\newthought{Both the amplitudes and multipliers equations} are formally identical to
...
\begin{equation}\label{eq:pte-s-cc}
  \begin{aligned}
  \hspace{-20pt}  {}^{\mathrm{PTE(S)}}\lag(\tamp{}, \tbar{},
  \p)_\mathcal{M} &=
  {}^\mathrm{PTE}\lag(\tamp{}, \tbar{})_\mathcal{M} \\
...
\acrshort{PTED}-\acrshort{CC} Lagrangian.
...
Moreover, as is the case \emph{in vacuo}, there are neither triples
amplitudes nor multipliers
to first order: $\tamp{\mu_3}^{[1]} = \tbar{\mu_3}^{[1]} = 0$.
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
\noindent\citeauthor{Koch1997-nm} showed that, assuming real wave functions, the
...
\arrowthought{The second order energy correction will be formed} including first order
...
\begin{equation}\label{eq:pted-mbpt-2nd}
  ...
  \frac{1}{2}{}^t\p^{[1]}\V\p^{[1]}
  +
  {}^t\p^{[1]}\braket{\mathrm{HF} | \commutator{\s}{T^{[1]}_1} |
  \mathrm{HF}} \\
  &= E^{[2]}
  \end{aligned}
\end{equation}
...
\arrowthought{The third order energy correction will be formed} including first order
...
\arrowthought{The fourth order energy corrections will be formed} including second
order amplitudes and polarization, while using first order multipliers:
\begin{equation}
  \begin{aligned}
    G^{[4]} &=
    \sum_{u=1}^2\braket{\tbar{u}^{[1]} | \commutator{\Phi}{T^{[2]}} |
    \mathrm{HF}} \\
    &+
  ...
  \end{aligned}
\end{equation}
The first two terms are recognized to be equal to $E^{[4]}$, so that:
\begin{equation}
  \begin{aligned}
    G^{[4]}
  &= E^{[4]}
...
\begin{equation}\label{eq:pted-mbpt-4th}
  ...
  &= E^{[4]}
    ...
    &+ \frac{1}{2}{}^t\p^{[2]}\left(\braket{\tbar{1}^{[1]}| \commutator{\s}{T^{[1]}} | \mathrm{HF} }
    + \braket{\tbar{2}^{[1]}|
    \commutator{\s}{T_2^{[1]}} | \mathrm{HF} } \right)\\
    &+
    {}^t\p^{[1]}
    \braket{\tbar{1}^{[1]} | \commutator{\s}{T_1^{[2]} + T_2^{[2]}} |
    \mathrm{HF}} \\
    &+
    \highlight{brewerRed}{
    {}^t\p^{[1]}
...
\arrowthought{Eventually, the fifth order energy correction is formed} including second order
amplitudes, multipliers and polarization:
\begin{equation}
  \begin{aligned}
    G^{[5]} &=
  ...
  \end{aligned}
\end{equation}
This can be rewritten in terms of $E^{[5]}$ and a number of polarization
terms:
\begin{equation}\label{eq:pted-mbpt-5th}
  ...
  \end{aligned}
\end{equation}
Finally, consideration of the excitation ranks involved in the
quantum/classical coupling terms yields
\begin{equation}\label{eq:pted-mbpt-5th}
  \begin{aligned}
    G^{[5]} &=
     E^{[5]}
    + {}^t\p^{[2]}\Bigg(\braket{\tbar{1}^{[1]} | \commutator{\s}{T_1^{[2]} +
    T_2^{[2]}} | \mathrm{HF}} \\
    &+ \highlight{brewerRed}{
    \braket{\tbar{2}^{[1]} | \commutator{\s}{T_2^{[2]} + T_3^{[2]}} | \mathrm{HF}}
    } \\
    &+ \braket{\tbar{1}^{[2]} | \commutator{\s}{T^{[1]}} | \mathrm{HF}}
    +
    \braket{\tbar{1}^{[1]} |
    \frac{1}{2}\commutator{\commutator{\s}{T_1^{[1]}}}{T_1^{[1]}}
    | \mathrm{HF}} \\
...
\begin{equation}\label{eq:triples-fifth-order}
  ...
  &+ {}^t\p^{[1]}\braket{\tbar{3}^{[2]} | \commutator{\s}{T_3^{[2]}} |
  \mathrm{HF}} \\
...
\arrowthought{In the \acrshort{PTE(S)} scheme,} the amplitudes equations are uncoupled
...
\arrowthought{In the \acrshort{PTES} scheme,} two different polarizations are
...
The amplitudes equations now become:
...
\begin{subequations}\label{eq:pted-cc2-multipliers}
  \begin{align}
    \denom{\mu_1}\tbar{\mu_1} &+
     \braket{\mathrm{HF} | \commutator{\check{\Phi}}{\cluster{\mu_1}} |
     \mathrm{HF}} \nonumber \\
   &+ \braket{\tbar{1} |
...
\arrowthought{We can construct the \emph{noniterative}}
...
The converged \acrshort{PTED}-\acrshort{CCSD} parameters are correct up
...
\begin{equation}
  ...
  +
  {}^t\p^{[2]} \braket{\tbar{2}^{[1]} | \commutator{\s}{T_3^{[2]}} |
  \mathrm{HF}} \\
  &+ O(6)
  \end{aligned}
\end{equation}
...
\newthought{Let us first consider the noniterative models for the inclusion of
connected triples.}
...
\arrowthought{The theory and implementation for iterative \acrshort{CC}
methods} with
