...
I will show a detailed derivation of the \acs{IEF} equation
for the \acs{PCM} transmission problem Eq.~\eqref{eq:transmission}:
\begin{subequations}
  \begin{align}[left={\empheqlbrace}]
...
\end{equation}
where the function $\sigma$ is the, yet unknown, \acl{ASC}.
...
\begin{empheq}[left={\empheqlbrace}]{align}
  ...
  \label{eq:fifth}
\end{empheq}
The final ingredient is the \ac{DtN} map, which can be derived by employing Eq.~\eqref{eq:stat3}
...
Let us now take a step back and formulate a different strategy for the
...
\ie the basic assumption behind \acs{COSMO}.
...
\todo[inline]{To write or not to write these????}
