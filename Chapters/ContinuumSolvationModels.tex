...
\begin{figure}[tb]
   ...
   Sparsity pattern achieved by the wavelet Galerkin \acs{BEM} when
...
Since the \acs{MEP} is a linear functional of the solute charge density,
the \acs{ASC} itself incurs in a dependency on the solute charge
density.
...
Why so? In such a framework the \acs{ASC} becomes an additional,
...
\acs{PCM} can be recast in a variational fashion yielding a true energy
functional of the \acs{ASC}:\autocite{Lipparini2010-be, Lipparini2011-aj, Lipparini2016-mo}
...
the \ac{PE},\autocite{Olsen2010-wa, Olsen2011-io} or
the \ac{FQ} model.\autocite{Rick1994-mn, Rick1995-wu, Rick1996-om,
Lipparini2011-rd}
...
\begin{equation}\label{eq:pcm-mm-stationarity}
  ...
  \end{pmatrix}
\end{equation}
While $\esp$ is quite clearly the \acs{MEP}, $\zeta$ can either be the
molecular electric field (MMpol and \acs{PE} models) or again the
\acs{MEP} (\acs{FQ} model).
...
field of \acs{MD} simulations.\autocite{Jadhao2012-gf, Jadhao2013-ry, Jadhao2013-hs,
Solis2013-ef}
