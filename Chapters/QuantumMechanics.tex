...
\footnotetext{--- electrons deify one razorblade \\
into a mountainrange; [...] \\
\hspace*{15pt} --- \textsc{E. E. Cummings}}
...
\begarrowthought{Quantum mechanics is the theory describing the motion and interactions
...
\begarrowthought{Molecular quantum mechanics is concerned with the microscopic motion of
...
\begarrowthought{In the language of second quantization}, we assume that a
...
\section{Mean-Field Theory}\label{sec:mean-field}
  ...
  \begarrowthought{The idea behind mean-field theory is simple:} seek the best description
...
\newthought{A different approach is taken in \gls{DFT}}, where the electron density
...
\newthought{How are \glspl{MO} constructed?} The usual approach is to expand them in
...
\newthought{We have already remarked that the \acrshort{CC} method is
not variational.}
...
\begarrowthought{Perturbation theory offers an alternative method to
recover the correlation energy} missing in the
...
\arrowthought{In the iterative schemes,} one approximates the amplitude equations for
...
\begin{equation}\label{eq:cc2-lagrangian}
  ...
  \end{aligned}
\end{equation}
The \acrshort{CC2} method brings the scaling of the \acrshort{CCSD}
...
The \gls{CC3} model is constructed analogously. Starting from the
...
\begin{equation}\label{eq:cc3-lagrangian}
  ...
  \end{aligned}
\end{equation}
The \acrshort{CC3} method brings the scaling of the \acrshort{CCSDT}
...
\begin{equation}\label{eq:cc3-expectation-value}
  \begin{aligned}
...
\arrowthought{Finally, one can construct \emph{noniterative} methods,} where the
