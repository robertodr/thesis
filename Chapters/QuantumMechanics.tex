...
\begin{itemize}
  \item nonrelativistic and relativistic Hamiltonians used in molecular
    quantum mechanics
  \item \gls{BO} approximation and the concept of \gls{PES}
  \item Fock space
  \item single-reference methods: \gls{HF} and \gls{KS} \gls{DFT}
  \item the \gls{CC}  model
\end{itemize}
...
\section{Single-Reference Methods}
...
also known as the \gls{BCH} expansion:
\begin{equation}
  \overline{O} =
...
Having summarized the essential points of \acrshort{CC} theory and having
introduced the relevant notation, we can proceed to analyze its coupling
with quantum/classical polarizable Hamiltonians.

\todo[inline]{Provide a primer here so that the Chapter on
solvation+correlation is not superlong}
