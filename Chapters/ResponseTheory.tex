...
\footnotetext{White light goin' messin' up my brain \\
White light it's gonna drive me insane \\
\hspace*{15pt} --- \textsc{The Velvet Underground}
}
...
\footnotetext{What's the frequency, Kenneth? \\
I never understood the frequency \\
\hspace*{15pt} --- \textsc{R.E.M.}}
...
\begarrowthought{Let us consider the experimentally relevant
situation} where a molecular
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=.7\textwidth]{Optical-rotation.pdf}
  \caption{
...
\arrowthought{Under the assumption that the external field is weak} with respect to the
...
\newthought{Having introduced response functions as a useful tool} to quantify the
...
\arrowthought{Let us consider in more detail the form of the linear
response function Eq. \eqref{eq:linres}.}
The formal solution to the response equation Eq.
...
\newthought{The derivation above holds for an exact state,} a situation that is never
...
\arrowthought{Solvent relaxation is an important point when formulating response
...
\newthought{The variational setting introduced in Section
\ref{sec:variational},}
...
\begin{equation}\label{eq:tilde-G00-a}
  ...
  \end{aligned}
\end{equation}
The integral derivative matrices involved in the previous expression
were introduced in \noparcite[ref.][]{Thorvaldsen2008-sg}.
